...
//        constraint.secondItem?.removeConstraint(constraint)
    ...
    private func createViews(keyboard: Keyboard) {
        for (pageIndex, page) in enumerate(keyboard.pages) {
            let numRows = page.rows.count
            
            for rowIndex in 0...numRows {
                var rowGap = Spacer(color: ((rowIndex == 0 || rowIndex == numRows) ? UIColor.purpleColor() : UIColor.yellowColor()))
                let rowGapName = "rowGap\(rowIndex)p\(pageIndex)"
                rowGap.setTranslatesAutoresizingMaskIntoConstraints(false)
                self.elements[rowGapName] = rowGap
                self.superview.addSubview(rowGap)
                
                if (rowIndex < numRows) {
                    let numKeys = page.rows[rowIndex].count
                    
                    for keyIndex in 0...numKeys {
                        var keyGap = Spacer(color: UIColor.blueColor())
                        let keyGapName = "keyGap\(keyIndex)x\(rowIndex)p\(pageIndex)"
                        ...
                        if (keyIndex < numKeys) {
                            var key = page.rows[rowIndex][keyIndex]
                            
                            var keyView = KeyboardKey(frame: CGRectZero, model: key) // TODO:
                            let keyViewName = "key\(keyIndex)x\(rowIndex)p\(pageIndex)"
    ...
    private func createKeyGapConstraints(keyboard: Keyboard) {
        for (pageIndex, page) in enumerate(keyboard.pages) {
            for rowIndex in 0..<page.rows.count {
                // TODO: both of these should be determined based on the model data, not the row #
                let isSideButtonRow = (rowIndex == 2)
                let isEquallySpacedRow = (rowIndex == 3)
                let rowsCount = page.rows[rowIndex].count
                ...
                if isSideButtonRow {
                    ...
                    addGapPair(
                        ...
                        row: rowIndex,
                        startIndex: 0,
                        endIndex: page.rows[rowIndex].count,
                        leftAnchor: "leftSpacer",
                        rightAnchor: "rightSpacer",
                        vertical: false,
                        width: "0")
